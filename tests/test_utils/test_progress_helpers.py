...
from app.utils.progress_helpers import (
    ...
    get_weekday_data, get_sample_weekday_data, get_trend_message, get_recent_entries
)
    ...
    def test_get_points_data_with_data(self, app, sample_user):
        ...
        with app.app_context():
            ...
            for i in range(3):
                stats = DailyStats(
                    user_id=sample_user.id,
                    date=date.today() - timedelta(days=2-i),  # Order: 2 days ago, 1 day ago, today
                    points=10,
                    current_streak=1,
                    longest_streak=1
                )
    ...
    def test_get_weekday_data_with_sufficient_data(self, app, sample_user):
        ...
        with app.app_context():
            ...
            for i in range(2):
                stats = DailyStats(
                    user_id=sample_user.id,
                    date=base_date - timedelta(days=(base_date.weekday() - 0) + i*7),  # Monday this week and last week
                    points=10,
                    current_streak=1,
                    longest_streak=1
                )
            ...
            for i in range(2):
                stats = DailyStats(
                    user_id=sample_user.id,
                    date=base_date - timedelta(days=(base_date.weekday() - 1) + i*7),  # Tuesday this week and last week
                    points=10,
                    current_streak=1,
                    longest_streak=1
                )
