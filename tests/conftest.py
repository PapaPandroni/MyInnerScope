...
def extract_csrf_token(response_data):
    ...
    if b"csrf-token" in response_data:
        ...
        match = re.search(
            b'<meta[^>]*name="csrf-token"[^>]*content="([^"]+)"', response_data
        )
        if match:
...
def sample_user(app):
    ...
    with app.app_context():
        user = User(
            email="test@example.com", password="testpassword123", user_name="Test User"
        )
        db.session.add(user)
        db.session.commit()
...
def sample_diary_entry(app, sample_user):
    ...
    with app.app_context():
        ...
        entry = DiaryEntry(
            user_id=user_id, content="This is a test diary entry", rating=5
        )
