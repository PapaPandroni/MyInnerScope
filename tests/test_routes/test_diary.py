...
class TestDiaryRoutes:
    ...
    def test_create_diary_entry_success_positive_rating(self, client, app, sample_user):
        ...
        assert response.status_code == 200
        assert b'Today I helped a friend with their project.' in response.data # Should show in recent entries

        with app.app_context():
            entry = DiaryEntry.query.filter_by(user_id=sample_user.id, content='Today I helped a friend with their project.').first()
            assert entry is not None
            assert entry.rating == 1
            assert entry.entry_date == date.today()

            stats = DailyStats.query.filter_by(user_id=sample_user.id, date=date.today()).first()
    ...
    def test_create_diary_entry_success_negative_rating(self, client, app, sample_user):
        ...
        with app.app_context():
            entry = DiaryEntry.query.filter_by(user_id=sample_user.id, content='I procrastinated on an important task today.').first()
            assert entry is not None
            assert entry.rating == -1
    ...
    def test_create_diary_entry_invalid_content_empty(self, client, app, sample_user):
        ...
        with app.app_context():
            entry = DiaryEntry.query.filter_by(user_id=sample_user.id, content='').first()
            assert entry is None # No entry should be created

    def test_create_diary_entry_invalid_content_too_long(self, client, app, sample_user):
        ...
        with app.app_context():
            entry = DiaryEntry.query.filter_by(user_id=sample_user.id, content=long_content).first()
    ...
    def test_create_diary_entry_invalid_rating(self, client, app, sample_user):
        ...
        with app.app_context():
            entry = DiaryEntry.query.filter_by(user_id=sample_user.id, content='Some content.').first()
            assert entry is None
    ...
    def test_diary_entry_updates_streak_new_day(self, client, app, sample_user):
        ...
        with app.app_context():
            # Day 1 entry
            entry1 = DiaryEntry(user_id=sample_user.id, content='Day 1', rating=1, entry_date=date.today() - timedelta(days=1))
            db.session.add(entry1)
            db.session.commit()
            
            stats1 = DailyStats(user_id=sample_user.id, date=date.today() - timedelta(days=1), points=5, current_streak=1, longest_streak=1)
        ...
        with app.app_context():
            stats2 = DailyStats.query.filter_by(user_id=sample_user.id, date=date.today()).first()
    ...
    def test_diary_entry_resets_streak_gap_day(self, client, app, sample_user):
        ...
        with app.app_context():
            # Day 1 entry
            entry1 = DiaryEntry(user_id=sample_user.id, content='Day 1', rating=1, entry_date=date.today() - timedelta(days=2))
            db.session.add(entry1)
            db.session.commit()
            
            stats1 = DailyStats(user_id=sample_user.id, date=date.today() - timedelta(days=2), points=5, current_streak=1, longest_streak=1)
            db.session.add(stats1)
            db.session.commit()
        ...
        with app.app_context():
            ...
            assert stats2.longest_streak == 1 # Longest streak remains 1 from previous entry
