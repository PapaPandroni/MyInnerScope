...
def test_delete_account_post_success(client, app):
    with app.app_context():
        ...
        user = User(
            email="delete@example.com", password="password", user_name="DeleteUser"
        )
        db.session.add(user)
        db.session.commit()
        ...
        response = client.post(
            url_for("user.delete_account"), data={"password": "password"}
        )
...
def test_delete_account_post_wrong_password(client, app):
    with app.app_context():
        ...
        response = client.post(
            url_for("user.delete_account"), data={"password": "wrong_password"}
        )
...
def test_profile_page_get(client, app):
    ...
    with app.app_context():
        # Create a test user
        user = User(
            email="profile@example.com", password="password", user_name="ProfileUser"
        )
        db.session.add(user)
        db.session.commit()
...
def test_change_username_success(client, app):
    ...
    with app.app_context():
        # Create a test user
        user = User(
            email="username@example.com", password="password", user_name="OldUsername"
        )
        db.session.add(user)
        db.session.commit()
        ...
        if b"csrf-token" in response.data:
            ...
            match = re.search(
                b'<meta[^>]*name="csrf-token"[^>]*content="([^"]+)"', response.data
            )
            if match:
...
def test_change_username_empty(client, app):
    ...
    with app.app_context():
        user = User(
            email="empty@example.com", password="password", user_name="TestUser"
        )
        db.session.add(user)
        db.session.commit()
...
def test_change_username_not_logged_in(client, app):
    ...
    response = client.post(
        url_for("user.change_username"), data={"new_username": "NewUsername"}
    )
    assert response.status_code == 302
    assert url_for("auth.login_page") in response.headers["Location"]
